...
class HomeController extends Controller
{
    public function index()
    {
        ...
        if (Navigation::query()->where('nav_category', 'Home')->where('nav_name', 'LIKE', "%steps%")->where('page_type', 'Group')->latest()->first() != null) {
            ...
            $steps = Navigation::query()->where('parent_page_id', $steps_id)->latest()->get();
            // return $steps;
        } else {
            $steps = null;
        }
        ...
        //return $missons;       
        return view("website.index")->with(['testimonial' => $testimonial, 'statistics' => $statistics, 'partners' => $partners, 'jobs' => $jobs, 'banners' => $banners, 'about' => $About, 'menus' => $menus, 'global_setting' => $global_setting, 'sliders' => $sliders, 'missons' => $missons, 'job_categories' => $job_categories, 'message' => $message, 'process' => $process, 'home_client' => $home_client, 'notice_data' => $notice_data, 'steps'=> $steps]);
    }
